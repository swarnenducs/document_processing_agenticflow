...
| Compact `provider:model` ids | **Yes** | `MAPPER_MODEL_ID=openai:gpt-5-mini` |
...
A: Env + `llm_factory` → LangChain `init_chat_model`. Prefer `MAPPER_MODEL_ID=openai:gpt-5-mini` or split `MAPPER_PROVIDER` / `MAPPER_MODEL`.
