...
ipp_agentic_api/             # FastAPI :8000 (Python package: ip_api)
UI/                          # Gradio :7860
...
Each of `document-processing-mcp`, `voice_enable_mcp`, `central-agentic-flow`, `ipp_agentic_api`, `UI` can be copied to its own repo. Each folder has `.env.example`, `requirements.txt`, `requirements-dev.txt`, `pyproject.toml`, `Dockerfile`, `run.py`, `run.sh`, and `tests/` (`pytest`).

```bash
cd document-processing-mcp   # or voice_enable_mcp / central-agentic-flow / ipp_agentic_api / UI
...
Postman (gateway + MAF, not Gradio/MCP JSON-RPC): [POSTMAN.md](POSTMAN.md). Split `ipp_agentic_api` repos can use [ipp_agentic_api/postman/](../ipp_agentic_api/postman/).
