...
| 3 | [03-voice-graph-hitl-memory.md](03-voice-graph-hitl-memory.md) — **HITL + MemorySaver** |
...
| **Checkpointer** | Persists graph state between turns (`MemorySaver`, Postgres, …) |
...
| Voice (`voice_enable_mcp`) | **Yes** — `MemorySaver` + `thread_id` for HITL |
