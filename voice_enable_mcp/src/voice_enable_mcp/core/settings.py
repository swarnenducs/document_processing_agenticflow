...
from voice_enable_mcp.core.dynaconf_loader import apply_dynaconf_from_env_files
...
class Settings(BaseSettings):
    ...
    checkpoint_backend: str = Field(
        default="sql",
        validation_alias=AliasChoices("LANGGRAPH_CHECKPOINT_BACKEND"),
    )
    redis_url: str | None = None
    checkpoint_redis_prefix: str = Field(
        default="lg",
        validation_alias=AliasChoices("LANGGRAPH_CHECKPOINT_REDIS_PREFIX"),
    )
    checkpoint_redis_ttl_seconds: int | None = Field(
        default=None,
        validation_alias=AliasChoices("LANGGRAPH_CHECKPOINT_REDIS_TTL_SECONDS"),
    )

    @field_validator("azure_sql_dialect", "speech_provider", "checkpoint_backend", mode="after")
    @classmethod
    def _lower_str(cls, value: str) -> str:
        return (value or "").strip().lower()

    @field_validator("checkpoint_redis_prefix", mode="after")
    @classmethod
    def _prefix_or_lg(cls, value: str) -> str:
        return (value or "").strip() or "lg"
